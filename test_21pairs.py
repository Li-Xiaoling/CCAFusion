...
from net import NestFuse_light2_nodense,Fusion_network_CCAF_cross,ir_salient_detection, Fusion_network, Fusion_strategy
import utils
...
def load_model(path_auto, path_fusion, fs_type, flag_img):
	...
	fusion_model = Fusion_network_CCAF_cross(nb_filter, fs_type)
	...
	fusion_strategy = fusion_strategy.cuda()
	...
	type_size = 4
	# APARA = para * type_size / 1000 / 1000
...
def run_demo(nest_model, fusion_model, fusion_strategy, infrared_path, visible_path, output_path_root, name_ir, fs_type, use_strategy, flag_img, alpha):
	...
	if c is 1:
		...
		if use_strategy:
			...
			# f = fusion_model(img_ir, en_r, en_v)
			# torch.save(mask, "mask.png")
		# decoder
		...
		for i in range(c):
			...
			if use_strategy:
				...
				f = fusion_model(en_r, en_v)
				# f = fusion_model(img_ir_temp, en_r, en_v)
			# decoder
	...
	with torch.no_grad():
		...
		for alpha in alpha_list:
			for w_all in w_all_list:
				...
				# load network
				path_fusion = path_fusion_root + str(w) + '/' + 'Final_epoch_' + str(alpha) + '_wir_' + str(w) + '_wvi_' + str(w2)+'.model'
